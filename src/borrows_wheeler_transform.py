'''
DISCLAMER:
We used this algorithm as a helper for the main algorithms,
so we did not implement it ourselvs but used an external implementation
'''
...
class BWT():
    ...
    def transform(self, txt: str):
        return encode_BWT(txt + "$")

    def restore(self, txt: str):
        return decode_BWT(txt)[:-1]
...
def DC3_samsort(suftuplst):
    ...
    while flag:
        ...
        # sort by ranks
        indices = sorted(invsuffixarray, key=cmp_to_key(lambda x,y:cmp(x,y)))
...
def DC3_radixsort(suftuplst, iternum):
    ...
    keysortedbuckets = sorted(buckets, key=cmp_to_key(lambda x,y:cmp(x,y))) # sort by numerical value
    # order the suffixes
    ordered = []
